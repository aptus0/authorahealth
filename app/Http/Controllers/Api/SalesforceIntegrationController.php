...
class SalesforceIntegrationController extends Controller
{
    ...
    public function install(Request $request): JsonResponse
    {
        ...
        $connection = $this->connection($request);
        abort_if(in_array($connection->provisioning_status, ['queued', 'validating'], true), 409, 'Provisioning is already in progress.');

        $connection->update(['provisioning_status' => 'queued', 'provisioning_progress' => 15, 'provisioning_step' => 'Salesforce validation queued.', 'last_error' => null]);
